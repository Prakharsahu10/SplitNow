...
export const createSettlement = mutation({
  ...
  handler: async (ctx: MutationCtx, args) => {
    ...
    // Find the current user by token identifier
    ...
    if (args.groupId) {
      const group = (await ctx.db.get(args.groupId)) as Group | null;
...
export const getSettlementData = query({
  ...
  handler: async (ctx: QueryCtx, args) => {
    ...
    if (args.entityType === "user") {
      ...
      // ---------- initialise per‑member tallies with proper typing
      ...
      for (const exp of expenses) {
        if (exp.paidByUserId === me._id) {
          ...
          const split = exp.splits.find((s) => s.userId === me._id && !s.paid);
          if (split && balances[exp.paidByUserId]) {
      ...
      const members = await Promise.all(
        memberIds.map((id) => ctx.db.get(id as Id<"users">))
      );

      const list = memberIds.map((uid) => {
        const m = members.find((u) => u && u._id === uid) as User | undefined;
