...
async function createGroups(ctx: MutationCtx, users: Doc<"users">[]) {
  const now = Date.now();
...
async function createOneOnOneExpenses(ctx: MutationCtx, users: Doc<"users">[]) {
  const now = Date.now();
...
async function createSettlements(
  ...
  const cabExpense = oneOnOneExpenses.find(
    (expense: { description?: string }) =>
      expense.description === "Cab ride to airport"
  );

  // Find some group expenses to settle
  const hotelExpense = groupExpenses.find(
    (expense: { description?: string }) =>
      expense.description === "Hotel reservation"
  );

  const coffeeExpense = groupExpenses.find(
    (expense: { description?: string }) =>
      expense.description === "Coffee and snacks"
  );
