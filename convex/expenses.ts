...
export const createExpense = mutation({
  ...
  handler: async (ctx: MutationCtx, args): Promise<Id<"expenses">> => {
    ...
    const totalSplitAmount: number = args.splits.reduce(
      (sum: number, split: ExpenseSplit) => sum + split.amount,
      0
    );
    const tolerance: number = 0.01; // Allow for small rounding errors
...
export const deleteExpense = mutation({
  ...
  handler: async (ctx: MutationCtx, args): Promise<{ success: boolean }> => {
    // Get current user using auth
    const identity = await ctx.auth.getUserIdentity();
    if (!identity) {
      throw new Error("Unauthenticated");
