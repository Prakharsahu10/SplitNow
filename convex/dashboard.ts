...
interface Group {
  ...
  name: string;
  description?: string;
  members: Array<{ userId: Id<"users">; role: string; joinedAt: number }>;
}
...
export const getUserBalances = query({
  handler: async (
    ...
    for (const [uid, { owed, owing }] of Object.entries(balanceByUser)) {
      ...
      const base = {
        userId: uid,
        name: (counterpart as User)?.name ?? "Unknown",
        imageUrl: (counterpart as User)?.imageUrl,
        amount: Math.abs(net),
      };
      if (net > 0) {
...
export const getMonthlySpending = query({
  handler: async (
    ...
    const result = Object.entries(monthlyTotals).map(([month, total]) => ({
      month: new Date(parseInt(month)).toLocaleString("default", {
        month: "short",
      }),
      amount: total,
    }));

    // Sort by month timestamp (ascending)
    result.sort((a, b) => {
