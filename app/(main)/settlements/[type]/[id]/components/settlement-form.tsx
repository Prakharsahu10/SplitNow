...
export default function SettlementForm({
  ...
  entityData,
  onSuccess,
}: SettlementFormProps): React.JSX.Element | null {
  ...
  const handleUserSettlement = async (data: FormData): Promise<void> => {
    if (!currentUser || !entityData || entityData.type !== "user") {
  ...
  const handleGroupSettlement = async (
    ...
    if (!currentUser || !entityData || entityData.type !== "group") {
    ...
    try {
      ...
      const selectedUser = entityData.balances.find(
        (balance: GroupBalance) => balance.userId === selectedUserId
      );
  ...
  if (entityType === "user" && entityData.type === "user") {
  ...
  if (entityType === "group" && entityData.type === "group") {
    ...
    return (
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="space-y-2">
          ...
          <div className="space-y-2">
            {groupMembers.map((member: GroupBalance) => {
