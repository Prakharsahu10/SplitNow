...
export function ParticipantSelector({
  ...
  const { data: searchResults, isLoading } = useConvexQuery(
    ...
    { query: searchQuery }
  ) as { data: SearchUser[] | null; isLoading: boolean };

  // Add a participant
  const addParticipant = (user: SearchUser): void => {
    // Check if already added
    if (participants.some((p) => p.id === user.id)) {
      return;
    }

    // Convert SearchUser to Participant format
  ...
  const removeParticipant = (userId: Id<"users">): void => {
